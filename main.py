class Student:
    ...
    def __str__(self):
        result = f'Имя: {self.name}\nФамилия: {self.surname}\n' \
                 f'Средний балл за домашние задания: {self.average_grades}\n' \
...
class Lecturer(Mentor):
    ...
    def __str__(self):
        result = f'Имя: {self.name}\nФамилия: {self.surname}\n' \
                 f'Средний балл за лекции: {self.average_grades_l}'
        return result
...
one_student = Student('Маргоро', 'Робби', 'ЖЕН')
one_student.courses_in_progress += ['Java', 'Python', 'C++']
two_student = Student('Брэд', 'Питт', 'МУЖ')
two_student.courses_in_progress += ['Java', 'Python', 'PHP']

one_mentor = Reviewer('Леонардо', 'ДиКаприо')
one_mentor.courses_attached += ['Java', 'Python']
two_mentor = Reviewer('Джонни', 'Депп')
two_mentor.courses_attached += ['Java', 'Python']
three_mentor = Lecturer('Шарлиз', 'Терон')
three_mentor.courses_attached += ['Java', 'Python']
four_mentor = Lecturer('Джим', 'Керри')
four_mentor.courses_attached += ['Java', 'Python']

one_mentor.rate_hw(one_student, 'Python', 8)
one_mentor.rate_hw(one_student, 'Python', 10)
one_mentor.rate_hw(two_student, 'Python', 7)
one_mentor.rate_hw(two_student, 'Python', 5)
two_mentor.rate_hw(one_student, 'Java', 10)
two_mentor.rate_hw(one_student, 'Java', 10)
two_mentor.rate_hw(two_student, 'Java', 8)
two_mentor.rate_hw(two_student, 'Java', 6)

one_student.rate_lectors(three_mentor, 'Python', 10)
one_student.rate_lectors(three_mentor, 'Python', 7)
one_student.rate_lectors(three_mentor, 'Python', 8)
one_student.rate_lectors(three_mentor, 'Python', 9)
two_student.rate_lectors(four_mentor, 'Java', 10)
two_student.rate_lectors(four_mentor, 'Java', 10)
two_student.rate_lectors(four_mentor, 'Java', 7)
...
two_student.completion_course('PHP')
...
def average_grade_in_subject(class_name, course):
    ...
    return f'Средний балл среди {class_name.__name__} по {course} - {average_grade}'
